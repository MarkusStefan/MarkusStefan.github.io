...
function parseRssItems(xml) {
  ...
  const pubDate = stripHtml(get('pubDate'));
    ...
    const thumbnail = imgMatch ? imgMatch[1] : '/images/placeholder.svg';
      ...
      try {
        ...
        if (res.ok) {
          ...
          const mapped = Array.isArray(data.items) ? data.items.map(it => {
            ...
            const thumb = it.thumbnail || (it.enclosure && it.enclosure.link) || (imgMatch ? imgMatch[1] : '/images/placeholder.svg');
            return {
              title: it.title,
              link: it.link,
